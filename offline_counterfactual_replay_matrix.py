...
from counterfactual_variant_utils import add_counterfactual_variant_flags
from offline_trajectory_utils import add_offline_analysis_flags, ordered_unique_ints


def build_parser():
    parser = ArgumentParser(
        "Run matched trajectory caching plus counterfactual same-trajectory replay with separate generator and evaluator checkpoint lists."
    )
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Checkpoint episodes used to generate matched trajectory caches.",
    )
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Checkpoint episodes used to replay trajectories and fit decoders.",
    )
    parser.add_argument(
        "--num_samples",
        type=int,
        default=10000,
        help="Target number of cached decision timesteps per generator checkpoint.",
    )
    parser.add_argument("--epsilon", type=float, default=0.0)
    parser.add_argument("--sample_wandb_project", type=str, default="matched-trajectory-cache")
    parser.add_argument("--replay_wandb_project", type=str, default="offline-counterfactual-replay-decode")
...
def main(args):
    ...
    sample_script = repo_root / "sample_matched_trajectories.py"
    replay_script = repo_root / "offline_counterfactual_replay_decode_eval.py"
